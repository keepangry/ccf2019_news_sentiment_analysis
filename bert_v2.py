...
# NaN给默认值
origin_train_data = origin_train_data.fillna("无")
origin_test_data = origin_test_data.fillna("无")
...
test_data_df = origin_test_data.fillna("无")
...
# 如果使用training=True就会报错，输入变成3个，应该是代表train语言模型的意思。
bert_model = load_trained_model_from_checkpoint(config_path, checkpoint_path, trainable=True, seq_len=None)
...
# train_data = [train_data[i] for i, j in enumerate(train_data) if i % 6 != 0]
train_data = [train_data[i] for i, j in enumerate(train_data) if i % 6 != 0]
valid_data = [train_data[i] for i, j in enumerate(train_data) if i % 6 == 0]

# train_data, valid_data = train_test_split(train_data, test_size=0.2, random_state=3)
...
if is_test:
    # 测试集
    fw = open('bert_trainable1_drop0.2_len150_segment12_0.986_20190827.csv', 'w')
    fw.write('id,label\n')
